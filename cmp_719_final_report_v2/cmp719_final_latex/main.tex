...
the conditioning. For frame-level comparability we read out the pedestrian
...
frame-level AUROC, and --- because overall AUROC mixes motion-intrinsic with
interaction anomalies --- a hazard-focused suite: a class-balanced
near-vehicle AUROC (bootstrap CI), the Spearman correlation between score and
pedestrian--vehicle proximity, and a counterfactual interaction probe
(Section~\ref{sec:results}). Multi-seed results use five seeds
$\{42,1,2,3,4\}$.
...
\label{sec:ablation}
Our variants form a coherent ablation (Tables~\ref{tab:single}--\ref{tab:hazard}).
\textbf{(i) Context value:} adding any context lifts mean AUROC by $+6$--$7$ pp
(baseline $\to$ proximity/car-matrix). \textbf{(ii) Spatial alignment
(shuffled control):} permuting the context across segments --- identical
parameter count, geometry misaligned --- drops the car-matrix best from $0.802$ to
$0.789$; the $\sim$1 pp aligned-vs-shuffled gap is the portion attributable to
genuine spatial alignment, the remainder to the regularising effect of a richer
input (a multi-seed shuffled run to put error bars on this gap is in progress).
\textbf{(iii) FiLM covariance:} the FiLM gate yields the best mean AUROC and the
strongest score--proximity coupling. \textbf{(iv) Representation:} the oriented
car-matrix and the simple proximity vector are comparable on overall AUROC,
indicating most of the overall gain is proximity-driven, while the
\emph{geometry} matters more for the hazard-specific metrics. \textbf{(v) Joint
vs.\ context model:} Scene-SKDE matches the context model on overall AUROC, i.e.\
the unified formulation is an elegant alternative rather than a clear overall
winner; its advantage --- modelling vehicle-intrinsic anomalies --- is exercised
only under a joint read-out (future work). \textbf{(vi) Temporal smoothing}
(Figure~\ref{fig:sigma}) is the most influential test-time hyper-parameter.
